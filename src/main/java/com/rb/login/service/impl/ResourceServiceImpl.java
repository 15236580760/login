...
public class ResourceServiceImpl implements ResourceService {
    ...
    public int save(Resource resource) {
        if(null == resource.getId()){
            //新建
            //验证名称是否重复。
            if(resourceMapper.countByUrlAndMethod(resource.getUrl(),resource.getMethod()) > 0){
                throw new BusinessException("资源url和method重复");
            }
            ...
            if(!oldResource.getUrl().equals(resource.getUrl()) || !oldResource.getMethod().equals(resource.getMethod())){
                if(resourceMapper.countByUrlAndMethod(resource.getUrl(),resource.getMethod()) > 0){
                    throw new BusinessException("资源url和method重复");
                }
            }
